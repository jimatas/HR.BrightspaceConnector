...
namespace HR.BrightspaceConnector.Infrastructure.Persistence
{
    public class Database : IDatabase
    {
        ...
        public async Task<SectionRecord?> GetNextCourseOfferingSectionAsync(CancellationToken cancellationToken = default)
        {
            var sprocName = $"sync_out_brightspace_{environment.GetStoredProcedureEnvironmentName()}_courseOfferingSection_GetNextEvents";
            logger.LogDebug("Executing stored procedure '{SprocName}'.", sprocName);

            var courseOfferingSections = await dbContext.CourseOfferingSections.FromSqlRaw(sprocName).AsNoTracking().ToListAsync(cancellationToken).WithoutCapturingContext();
            return courseOfferingSections.SingleOrDefault();
        }
