    ...
    internal class OrgUnitRecordConfiguration : IEntityTypeConfiguration<OrgUnitRecord>
    {
        public void Configure(EntityTypeBuilder<OrgUnitRecord> builder)
        {
            ...
            builder.Property(ou => ou.Parents).IsRequired(false).HasConversion(
                ...
                convertFromProviderExpression: (string? value) => !string.IsNullOrEmpty(value) ? value.Split(',', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries).Select(int.Parse) : Enumerable.Empty<int>(),
